...
from voyagr.services.routing.costing import build_auto_costing_options, VALID_ROUTE_OPTIMIZATIONS
from voyagr.utils import validate_coordinates, normalize_vehicle_type
...
def build_multidrop_route_from_request(data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
    """
    Handle an /api/multi-stop-route request body end to end.

    Parses the request payload, assembles the stop list, optionally fetches
    hazards / CAZ exclude points, delegates to ``build_multidrop_route`` and
    formats the response. Returns ``(response_dict, http_status)`` and never
    raises (mirrors the previous inline Flask handler, including status codes).
    """
    try:
        ...
        if len(all_stops) < 1:
            return {'success': False, 'error': 'Need at least 1 stop'}, 400
        if len(all_stops) > 25:
            return {'success': False, 'error': 'Maximum 25 stops allowed'}, 400
        ...
        if not all_stops:
            return {'success': False, 'error': 'Need at least 1 stop besides start/end'}, 400
