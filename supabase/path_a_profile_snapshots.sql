-- Voyagr Path A (Dual DB) - Supabase schema for per-user profile snapshots
--
-- Purpose:
-- Store a user's PWA "profile snapshot" (settings + saved routes) in Supabase Postgres,
-- while keeping cameras + GraphHopper local on Contabo.
--
-- Run this in Supabase SQL Editor (project: your Voyagr project).
...
on public.voyagr_profile_snapshots
for select
