...
// Derived from sherpa-onnx wasm/kws/app.js and MDN web-dictaphone patterns.
// Source: https://github.com/k2-fsa/sherpa-onnx/tree/master/wasm/kws/app.js
...
Module.onRuntimeInitialized = async function () {
  console.log('[Sherpa KWS spike] WASM runtime ready');
  try {
    const kwResp = await fetch('/static/vendor/sherpa-kws/spike-config/keywords-hey-sat-nav.txt');
    if (!kwResp.ok) {
      throw new Error('Missing keywords file (HTTP ' + kwResp.status + ')');
    }
    const keywordsText = (await kwResp.text()).trim();
    const myConfig = {
    ...
    recognizer = createKws(Module, myConfig);
    hint.innerText = 'Model loaded — click Start, then say «Hey Sat Nav».';
    startBtn.disabled = false;
    console.log('[Sherpa KWS spike] recognizer ready', recognizer);
  } catch (e) {
    console.error(e);
    hint.innerText = 'Init failed: ' + (e && e.message ? e.message : String(e));
  }
};
